#!/usr/bin/env bash
# dns-sync.sh — sync Porkbun DNS A records to the VPS IP via the direct API.
#
# Why not the tofu provider: marcfrederick/porkbun's create path is broken
# upstream (the API returns the record id as an object; the provider parses it
# as int64 — issue #35, unfixed as of v1.3.3). DNS is therefore managed with
# the raw Porkbun API instead.
#
# Ensures an A record for the apex domain and every subdomain points at the
# VPS IP. Idempotent: edits existing records whose content differs, creates
# missing ones. Safe to run on every deploy.
#
# Usage:
#   scripts/dns-sync.sh              # sync apex + subdomains to VPS IP
#   scripts/dns-sync.sh --help       # this message
#
# Environment (all from .rendered/vault.env when present):
#   PORKBUN_API_KEY, PORKBUN_SECRET_API_KEY   API credentials
#   DOMAIN, SUBDOMAINS_JSON                   what to sync (apex + subdomains)
#   VPS_IP | VPS_HOST                         target IP (default: tofu output)
#   DNS_TTL                                   TTL seconds (default: 600)
...
# Load vault-derived env (PORKBUN_API_KEY, PORKBUN_SECRET_API_KEY, DOMAIN, SUBDOMAINS_JSON)
...
# --- build target list: apex + each subdomain -------------------------------
TARGETS=("$DOMAIN")
...
# --- ensure each record -----------------------------------------------------
# Porkbun's edit endpoint is broken upstream (returns
# EDIT_ERROR_WE_WERE_UNABLE_TO_EDIT_THE_DNS_RECORD for every request), so we
# reconcile with delete + create instead: keep exactly one A record per name
# pointing at the target IP, dropping stale duplicates.
rc=0
for name in "${TARGETS[@]}"; do
  # all A records for this name (apex records carry the bare domain as name)
