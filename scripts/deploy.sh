...
#   - Cloudflare API token (RCLB_DEV_CLOUDFLARE_API_KEY in Bitwarden SM)
#   - SSH key uploaded to OVH account
#   - Bitwarden CLI installed with Secrets Manager access token configured
#
# Usage:
...
# Load Secrets Manager-derived secrets (SSH_PRIVATE_KEY, VPS_HOST, etc.) if available.
...
phase_inventory_render() {
  ...
  # Pull deployment config from Secrets Manager into .rendered/vault.env.
...
phase_tofu() {
  ...
  # Inventory is now rendered from Secrets Manager by phase_inventory_render()
...
phase_infect() {
  ...
  # Ubuntu 24.04+ mounts /tmp as tmpfs, so nixos-infect's swapfile setup
  # (swapon on /tmp/nixos-infect.*.swp) fails with "Invalid argument" and
  # aborts under set -e. Route the swapfile to disk-backed /var/tmp instead.
...
phase_secrets_render() {
  ...
  # Ensure SM access token is configured on workstation.
  # SM has no interactive login — the token must be set via BWS_ACCESS_TOKEN
  # env or ~/.config/bitw/config (key: sm_access_token).
  if ! bws_check; then
    exit 1
  fi
...
phase_dns() {
  # DNS records are managed by OpenTofu via the Cloudflare provider
  # (tofu/dns.tf). The tofu apply phase already created/updated them;
  # this phase is a no-op that verifies the legacy script's replacement.
  # Legacy scripts/dns-sync.sh (Porkbun) is kept for reference only.
...
main() {
  ...
  # do not need the vault, so skip them to avoid unnecessary Secrets Manager calls.
  case "${1:-}" in
