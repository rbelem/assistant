...
# 2. Commit in the mirror (if anything changed)
# 3. Print the push command (user does this; dcg blocks force-pushes)
# 4. Wait for the user to push + watch gitleaks
# 5. Once gitleaks is green, run fetch_vault.sh
# 6. Run tofu init / plan / apply
# 7. Run the initial deploy (nixos-infect + first nixos-rebuild as root)
# 8. Verify
#
# All destructive operations (tofu apply, push) are gated behind user
# confirmation. dcg blocks the AI from running them, but the user
# (with their master password and ssh-agent) can proceed.
#
# NOTE: The pre-push hook at .git/hooks/pre-push automatically triggers
# the mirror sync when you run `git push` to main. This hook is
# worktree-local (not in git), so after a fresh clone you need to
# re-apply it from scripts/devbox/pre-push-hook (or copy manually).
...
# ---- Phase 3: print the push command ----
