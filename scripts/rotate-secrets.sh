...
***REMOVED*** Rotated values are mirrored to Bitwarden Secrets Manager (bws) — SM is the
***REMOVED*** render source, so a failed SM sync is fatal.
***REMOVED***
***REMOVED*** Usage:
...
***REMOVED*** shellcheck source=lib/bws.sh
...
***REMOVED***--- SM sync after rotation ---------------------------------------------------
***REMOVED*** Maps personal vault item names to SM key names and updates the SM secret.
_bws_map_sm_key() {
  local item_name="$1"
  case "$item_name" in
...
_bws_sync_after_rotate() {
  ...
  [[ -z "$sm_key" ]] && return 0  ***REMOVED*** no SM mapping for this item

  ***REMOVED*** Load token: env (BWS_ACCESS_TOKEN / legacy SM_ACCESS_TOKEN), else ~/.config/bitw/config
...
rotate_item() {
  ...
  ***REMOVED*** Sync the new value to the corresponding SM secret (fatal: SM is the render source)
  ...
  ***REMOVED*** NOTE: The corresponding SM secret is synced automatically by
  ***REMOVED*** _bws_sync_after_rotate() above. No manual update needed.
}
