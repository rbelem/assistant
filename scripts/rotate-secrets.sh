...
# Manual-only items (stateful): restic, n8n-key, postgres, zitadel-masterkey
#
...
#   rotate-secrets.sh status                            # list + exit 1 if any past age
#   rotate-secrets.sh generate <item> [--yes] [--dry-run]
#   rotate-secrets.sh generate --all [--yes] [--dry-run]
#   rotate-secrets.sh rotate <item> [--yes] [--dry-run] [--i-know-what-im-doing]
#   rotate-secrets.sh rotate --all [--yes] [--dry-run]
#   rotate-secrets.sh --help
#
# filter-repo will turn `#` into `***REMOVED***` in public mirror — project convention.
...
#--- PASSWORD resolution + bitw preflight -------------------------------------
# PASSWORD env overrides libsecret auto-unlock. Master password lives on
# workstation only — never on the VPS.
...
read_history() {
  ...
  if ! echo "$payload" | jq -e 'type == "array"' >/dev/null 2>&1; then
    # No-clobber guard: parse failed → backup raw, alert, abort
...
write_history() {
  ...
  # bitw edit takes --notes directly; no /proc cmdline exposure for secrets
  # (notes are passed as argv, but the history JSON contains no high-value secrets).
...
bootstrap_item_baseline() {
  ...
  # Check if item already has history entry
  local existing
  ...
  # Add bootstrap entry (no value change, just timestamp baseline)
  local new_entry
...
rotate_item() {
  ...
  history_json="$(read_history)"  # re-read after bootstrap
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    ...
    info "  new value length: ${#new_value} chars"
    return 0
  fi
  ...
  # Write new value to BW via --password-stdin (keeps secret out of /proc cmdline)
  ...
  if [[ "$verify_value" != "$new_value" ]]; then
    ...
    # Rollback: restore old value via --password-stdin
...
cmd_status() {
  ...
  # Check if any were past due (subshell issue — re-check)
  echo "$CONFIG_JSON" | jq -r '.items[] | "\(.name)\t\(.max_age_days)"' | \
  while IFS=$'\t' read -r name max_age; do
    local history_json
    history_json="$(read_history)"
...
cmd_generate() {
  ...
  if [[ "$TARGET_ITEM" == "--all" ]]; then
    # Rotate all auto items past age
    local history_json
    history_json="$(read_history)"
    ...
    while IFS=$'\t' read -r name category max_age; do
      if [[ "$category" == "manual" && "$I_KNOW" -ne 1 ]]; then
        continue  # skip manual unless --i-know-what-im-doing
      fi
