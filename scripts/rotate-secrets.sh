...
write_history() {
  ...
  # bw edit requires full item JSON via stdin
  local full_item
  full_item="$(bw_sesh get item "$item_id")"

  # /proc/<pid>/cmdline exposure: bw edit takes JSON as positional arg or stdin.
  # We use stdin via pipe to avoid cmdline exposure. On single-user box this is
...
rotate_item() {
  ...
  # Write new value to BW
  # bw edit requires full item JSON. Read → modify → write back.
  ...
  # /proc/<pid>/cmdline exposure: passing JSON via stdin pipe, not arg.
  local tmp
  tmp="$(mktemp)"
  trap 'rm -f "$tmp"' EXIT
