...
#   - bitw binary (VPS should not have bitw installed)
#   - bws binary (VPS should not have the SM CLI installed)
#   - BWS_ACCESS_TOKEN env var (SM access token never touches the VPS)
#
# This is a security gate: the master password never touches the VPS.
# Secrets are rendered on the workstation and applied via ansible.
#
# Usage:
#   scripts/verify-g3.sh              # verify VPS is clean
#   scripts/verify-g3.sh --help       # this message
#
# Environment:
#   VPS_IP      VPS IP address (default: from tofu output or inventory)
#   SSH_KEY     SSH private key (default: from .rendered/vault.env or ~/.ssh/id_ed25519)
#   SSH_USER    SSH user (default: from inventory or rodrigo)
...
# Source vault.env if available (for SSH_PRIVATE_KEY)
...
# Run verification checks on VPS
# Each check prints its status; we collect failures and report at the end.
FAILURES=0
